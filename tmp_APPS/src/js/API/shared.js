...
function already_done() {
   ...
   if(readCookie(PAGE_MODIFIED)!=null && readCookie(PAGE_MODIFIED)==window.location.href && $('#dejavu').length==0) {
      $('<div id="dejavu" class="alert alert-info m-4"><span class="icon-notification"></span> <b>You have already made edits for this page within the last hour. Please allow for at least one hour for changes to take effect.</b></div>').insertBefore('#main_container');
