...
def create_json_index_minute_day(day,month, year):
   ...
   for minute_stack in sorted(glob.iglob(main_dir + '*' + os.sep + '*' + MINUTE_STACK_EXT + '*', recursive=True), reverse=True):	
      ...
      # Add to cam_id
      try:
         index_day['hours'][int(analysed_minute['hour'])]['cam_id'][int(analysed_minute['cam_id'])]
      except:
         index_day['hours'][int(analysed_minute['hour'])]['cam_id'][int(analysed_minute['cam_id'])] = []


      if(cur_stack_data):
         index_day['hours'][int(analysed_minute['hour'])]['cam_id'][int(analysed_minute['cam_id'])].append(cur_stack_data)
