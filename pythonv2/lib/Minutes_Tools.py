...
def create_json_index_minute_day(day,month, year):
   ...
   all_minutes_in_folder = sorted(glob.iglob(main_dir + '*' + os.sep + '*' + MINUTE_STACK_EXT + '*', recursive=False), reverse=True)
 
   for minute_stack in all_minutes_in_folder:	
      ...
      added = False
      for c,t in enumerate(stacks_per_hours):
         if(int(t['h']) == int(analysed_minute['hour'])-1): 
            for ccam in t['cam']:
               if(ccam['f']==analysed_minute['cam_id'] and cur_stack_data != {}):
                  ccam['stacks'].append(cur_stack_data)
                  cur_stack_data = {}
                  added = True
                  break
            if(added is True):
