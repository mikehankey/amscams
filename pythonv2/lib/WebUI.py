...
def meteors_new(json_conf,form):  
   ...
   if len(meteors)>=1 :
      ...
      if(has_limit_day==0):
         pagination = get_pagination(cur_page,len(all_meteors),"/pycgi/webUI.py?cmd=new_meteors&meteor_per_page="+nompp,nompp)
      else:
         pagination = get_pagination(cur_page,len(all_meteors),"/pycgi/webUI.py?cmd=new_meteors&limit_day="+limit_day+"&meteor_per_page="+nompp,nompp)
