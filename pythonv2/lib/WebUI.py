...
def meteor_index(json_conf, form):
   ...
   for day in sorted(mi, reverse=True):
      for meteor_file in mi[day]:
         ...
         if show == 1:
            print("<tr class='" + color + "'><td><div class='st'></div></td><td> {:s}</td><td>{:s}</td><td>{:s}</td><td>{:s}</td><td>{:s}</td><td>{:s}</td><td>{:s}</td><td> {:s}</td><td> {:s} </td><td></td><td></td><td></td></tr> ".format(link, str(mi[day][meteor_file]['reduced']), multi_text, az_el, pos, pxs, str(ts), str(mi[day][meteor_file]['total_res_px'])[0:5], str(mi[day][meteor_file]['total_res_deg'])[0:5]))
   print("<tbody></table>")
...
def hd_cal_index(json_conf, form):
   ...
   for day in sorted(ci,reverse=True): 
      ...
      for cam_id in sorted(ci[day],reverse=False):
         ...
         if show_row == 1:
            print("<tr class='" + color + " clickable'  data-toggle='collapse' data-target='#fr"+div_id+"' aria-expanded='false' aria-controls='fr"+div_id+"'><td><div class='st'></div></td><td>{:s}</a></td><td>{:s}</td><td>{:s}</td><td>{:s}</td><td>{:s}</td><td>{:s}</td><td>{:s}</td><td>{:s}</td><td>{:s}</td></tr>".format(show_link, str(cam_day_sum[day][cam_id]['files_with_stars']), str(cam_day_sum[day][cam_id]['files_without_stars']), str(cam_day_sum[day][cam_id]['total_stars_tracked_for_night']), az_el, pos_ang, px_scale, str(cam_day_sum[day][cam_id]['avg_res_px_for_night'])[0:5],str(cam_day_sum[day][cam_id]['avg_res_deg_for_night'])[0:5]))
