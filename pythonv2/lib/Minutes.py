...
def get_min_details(cam_id,year,month,day,minute_file):
   return MINUTE_FOLDER +  os.sep + str(year) + os.sep + str(month)+'_'+str(day) + os.sep +str(year)+'_'+str(month)+'_'+str(day)+minute_file.replace(':','_').replace('.','_') + '_' + cam_id + '-' + MINUTE_STACK_EXT + '.png'
...
def get_minute_index_res(selected_start_date, selected_end_date,selected_period,selected_cam_ids):
   ...
   while(json_index is not None and cur_date>=selected_start_date):
      ...
      for cam in json_data['cams']:
         for _min in cam['min']:
            print(cam)
            #print(get_min_details(cam,str(date.year),str(date.month).zfill(2),str(date.day).zfill(2),_min))
            print("<br>")
