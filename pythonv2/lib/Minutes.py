...
def create_minute_html_res(res,cam_ids,year,month,day):
   ...
   for cam_id in cam_ids:
      cam_title += "<div style='width:"+ str(float(100/how_many_cams)) +"px'><h2>Cam#" + str(cam_id) + "</h2></div>" 
   ...
   while(we_have_res==1):
      ...
      for cam_id in cam_ids:
         ...
         if(cam_res is not False):
            ...
            toReturn += "<div style='padding: 0 1rem 1rem 0;'><img src='"+t+"' class='img-fluid cam_"+str(cam_id)+"'/>"+cam_res+"</div>"
         else:
            toReturn += "<div>X</div>"
            how_many_false+=1
