      ...
      for cam_id in cam_ids:
         ...
         if(cam_res is not False):
            ...
            if(how_many_meteors!=0):
               extra_class = 'meteor'
            toReturn += "<div class='"+extra_class+"' style='padding: 0 1rem 1rem 0;'><a href='webUI.py?cmd=minute_details&stack="+t+"'><img src='"+t+"' style='width: 100%; max-width: 350px;height: 169px;' data-rel='"+cam_res+"' class='img-fluid cam_"+str(cam_id)+"'/></a><span style='font-size:.75rem'>"+cam_res+"</span></div>"
         else:
