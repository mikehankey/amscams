      ...
      for cam_id in cam_ids:
         ...
         if(cam_res is not False):
            ...
            if(how_many_meteors!=0):
               extra_class = 'meteor'
            toReturn += "<div  style='padding: 0 1rem 1rem 0;'><a class='d-block  minute "+extra_class+"' href='webUI.py?cmd=minute_details&stack="+t+"'><img src='"+t+"' data-rel='"+cam_res+"' class='img-fluid cam_"+str(cam_id)+"'/></a><span style='font-size:.75rem'>"+cam_res+"</span></div>"
         else:
