...
def crop_video_keep_meteor_centered(json_file,video,w=FRAME_THUMB_W,h=FRAME_THUMB_H):
   ...
   if('frames' in data):
      ...
      for i in range(1,first_index):
         crop = new_crop_thumb(folder_path + os.sep + "frames" + str(i).zfill(5) + ".png",data['frames'][1]['x'],data['frames'][1]['y'],folder_path + os.sep + "frames" + str(i).zfill(5) +"X.png",True)
      ...
      cmd = "ffmpeg -y -f image2 -framerate 1 -r 3 -i " +  ' '.join(cropped_frames) + " " + video.replace('.mp4','-cropped.mp4')
      print(cmd)
