...
def get_results_from_date_from_monthly_index(criteria,start_date,end_date,max_res_per_page,cur_page): 
   ...
   while(json_index!=False):
      ...
      for day in kk:
            ...
            all_days_details[str(cur_year)+'/'+str(cur_month)+'/'+str(day)] = len(day)
...
def archive_listing(form): 
   ...
   if(len(res)==0): 
      ...
      if(criteria_text==''):
         ...
         template = template.replace("{FOUND}", "<div style='background-color: transparent; line-height: 1rem; margin: 1rem 3rem;' class='alert alert-info page_h ml-3'>Displaying " + str(len(res)) + " out of " +  str(total)  + " detections. Your criteria: "+criteria_text+"  " + found_text + "</div>")
      ...
      if(criteria_text==''):
         ...
         template = template.replace("{FOUND}", "<div style='background-color: transparent; line-height: 1rem; margin: 1rem 3rem;' class='alert alert-info page_h ml-3'>Displaying only 1 detection. Your criteria: "+criteria_text+"  " + found_text + "</div>")
      ...
      if(criteria_text==''):
         ...
         template = template.replace("{FOUND}", "<div style='background-color: transparent; line-height: 1rem; margin: 1rem 3rem;' class='alert alert-info page_h ml-3'>Displaying all " + str(len(res)) + " detections. Your criteria: "+criteria_text+"  " + found_text + "</div>")
