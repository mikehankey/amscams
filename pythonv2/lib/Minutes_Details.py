...
def minute_details(form):
   ...
   video_full_path  += analysed_minute['full'].replace('.png','.mp4').replace(analysed_minute['sec'],'*').replace(analysed_minute['ms'],'*').replace(analysed_minute['cam_id'],analysed_minute['cam_id']+'-trim*').replace(MINUTE_TINY_STACK_EXT,'').replace("-trim",'*')
