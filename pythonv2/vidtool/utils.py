...
def get_sd_frames(camID,date):
    ...
    onlyfiles = [f for f in listdir(cur_path) if camID in f and "-tn" not in f and "-night" not in f "trim" not in f and isfile(join(cur_path, f))]
    return(sorted(onlyfiles), cur_path)
 
#Input list of SD files, path of the current image
#Output Video of resized frames
def create_sd_vid(frames, path, date): 
    ...
    cmd = 'ffmpeg -hide_banner -loglevel panic -r 45 -f image2 -s 1920x1080 -i ' + newpath+ '/%d.png -vcodec libx264 -crf 25 -pix_fmt yuv420p '+newpath+'/'+date +'.mp4'
    output = subprocess.check_output(cmd, shell=True).decode("utf-8")
    #print(output)
    print('VIDEO READ AT '+newpath+'/'+date +'.mp4' )

files, path, date = get_sd_frames("010034","2019_06_23")
