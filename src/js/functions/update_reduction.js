function update_reduction_on_canvas_and_table(json_resp) {
    ...
    $.each(smf, function(i,v){
        ...
        table_tbody_html+= '<tr id="fr_'+frame_id+'" data-meteor-pos="'+parseFloat(v[2]/2)+','+parseFloat(v[3]/2)+'"><td><img alt="Thumb #'+frame_id+'" src='+thumb_path+' width=50 height=50 class="img-fluid select_meteor"/></td>';
        table_tbody_html+= '<td>'+frame_id+'</td><td>'+v[0]+'</td> <td>'+ parseFloat(v[2]/2)+'/'+parseFloat(v[3]/2)+'</td><td>'+v[4]+'</td><td>'+v[5]+'</td>';
