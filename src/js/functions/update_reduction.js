function update_reduction_on_canvas_and_table(json_resp) {
    ...
    $.each(smf, function(i,v){
        ...
        if(i==0) {
            ...
            table_tbody_html+= '<td class="position-relative"><a class="btn btn-success btn-sm select_meteor"><i class="icon-target"></i></a><a title="Add a frame" class="btn btn-primary btn-sm btn-mm" data-rel="'+ (frame_id-1) +'"><i class="icon-plus"></i></a><a class="btn btn-primary btn-sm btn-pp" title="Add a frame"  data-rel="'+ (frame_id+1) +'"><i class="icon-plus"></i></a></td>';
        } else {
            // We add a "+" after
