...
def optimize_var(var_name, cal_fn, cp,json_conf,rval,percision, cal_img):
   orig_cp = cp.copy()
   ...
   for i in range (-20,20):
      ...
      cp, bad_stars, marked_img = eval_cal_res(cal_fn, json_conf, tcal, cal_img,None,None,tcal['cat_image_stars'])
      #print("RA, DEC", tcal['ra_center'], tcal['dec_center'])
      if SHOW == 1:
         extra_text = "optimize " + var_name + " " + str(tcal[var_name])
         view_calib(cal_fn,json_conf, tcal,cal_img, show = 0)


      #print(var_name, orig_cp[var_name], tcal[var_name], val, tcal['total_res_px'])
      if cp['total_res_px'] < best_val:
         #print("BETTER:",cp['total_res_px'], " < ", best_val, start_res)
         best_cp = cp
         best_val = cp['total_res_px']
   return(best_cp)
...
def refit_meteor(meteor_file, con, cur, json_conf, mcp = None, last_best_dict = None):
   ...
   if False:
      ...
      for row_data in range_data:
         ...
         default_cp['cat_image_stars'], default_cp['user_stars'], flux_table = get_image_stars_with_catalog(meteor_file, default_cp, show_img)
...
def batch_review(station_id, cam_id, con, cur, json_conf, limit=10, work_type="top"):
      ...
      if len(close_stars) == 1:
         for star in close_stars[0:1]:
            ...
            if new_cat_x is not None and img_x is not None:
               ...
               match_dist = angularSeparation(ra,dec,img_ra,img_dec)
               if res_px < 50:
                  print("GOOD RES PX/ MATCH DIST:", res_px, match_dist)
                  ...
                  print("FAIL RES PX/ MATCH DIST:", res_px, match_dist)
...
def delete_bad_stars (cal_fn, cal_params, con,cur,json_conf, oimg):
   ...
   tb = pt()
   tb.field_names = ["Action", "Star", "Mag", "Intensity", "Center Dist", "Res Limit", "Res Pixels"]
   ...
   for star in cal_params['cat_image_stars']:
      ...
      if res_px < dist_limit :
         ...
         dvals = [ra, dec, cal_fn]
         cur.execute(sql, dvals)
      row = [act, dcname, mag, bp, int(center_dist), int(dist_limit), res_px]
      tb.add_row(row)
   print(tb)
   con.commit()
   cal_params['cat_image_stars'] = new_stars
...
def apply_calib (cal_file, calfiles_data, json_conf, mcp, last_cal_params=None, extra_text= "", do_bad=False, flux_table=None):
      ...
      if res_px > 8: 
         ...
         if rev_cal_params is not None:
            for key in rev_cal_params:
               cal_params[key] = rev_cal_params[key]
      ...
      # set to last known if dealing with bad cal? 
      if False:
      ...
      # is this a duplicate to the pair command? and also the get_image_stars command?
      #catalog_cat_image_stars, catalog_user_stars, flux_table = get_image_stars_with_catalog(cal_file, cal_params, oimg, flux_table)
      ...
      # ADD MORE STARS ?
      if True:
         cal_params['cat_image_stars'] = pair_star_points(cal_fn, oimg, cal_params, json_conf, con, cur, mcp)
         print("NEW CAT IMAGE STARS:", cal_params['cat_image_stars'])
         temp_stars = []
         for star in cal_params['cat_image_stars']:
            dcname,mag,ra,dec,img_ra,img_dec,match_dist,org_x,org_y,img_az,img_el,new_cat_x,new_cat_y,six,siy,cat_dist,star_int = star
            if star_int > 30:
               temp_stars.append(star)
               print("KEEY", dcname, mag, star_int, cat_dist)
            else:
               print("FAIL", dcname, mag, star_int, cat_dist)
         cal_params['cat_image_stars'] = temp_stars
      cal_params['cat_image_stars'] = temp_stars  
      print("CIS:", len(cal_params['cat_image_stars']) )
      debug_image(cal_params, oimg)
      ...
      if cal_params['total_res_px'] > 2:
         ...
         ores = cal_params['total_res_px']
         cp = optimize_var("pixscale", cal_fn, cal_params,json_conf,rval, percision, cal_img)
         if cp is not None:
            if cp['total_res_px'] < cal_params['total_res_px']:
               cal_params = cp
               #print("FRES:", cal_params['total_res_px'])
         cp = optimize_var("center_az", cal_fn, cal_params,json_conf,rval, percision, cal_img)
         if cp is not None:
            if cp['total_res_px'] < cal_params['total_res_px']:
               cal_params = cp
               #print("FRES:", cal_params['total_res_px'])
         cp = optimize_var("center_el", cal_fn, cal_params,json_conf,rval, percision, cal_img)
         if cp is not None:
            if cp['total_res_px'] < cal_params['total_res_px']:
               cal_params = cp
               #print("FRES:", cal_params['total_res_px'])
         cp = optimize_var("position_angle", cal_fn, cal_params,json_conf,rval, percision, cal_img)
         if cp is not None:
            if cp['total_res_px'] < cal_params['total_res_px']:
               cal_params = cp
               #print("FRES:", cal_params['total_res_px'])
...
def debug_image(cal_params, cal_img):
   ...
   for star in cal_params['cat_image_stars']:
      dcname,mag,ra,dec,img_ra,img_dec,match_dist,org_x,org_y,img_az,img_el,new_cat_x,new_cat_y,six,siy,cat_dist,star_int = star
      if cat_dist < mres:
         print("GOOD STAR:", dcname,six,siy,star_int, cat_dist)
      else:
         print("BAD STAR:", dcname,six,siy,star_int, cat_dist)
...
def recenter_fov(cal_fn, cal_params, cal_img, stars, json_conf, extra_text="", this_poly_in=None):
   (f_datetime, cam_id, f_date_str,fy,fmin,fd, fh, fm, fs) = convert_filename_to_date_cam(cal_fn)
   ...
   for star in cal_params['cat_image_stars']:
      dcname,mag,ra,dec,img_ra,img_dec,match_dist,new_x,new_y,img_az,img_el,new_cat_x,new_cat_y,six,siy,cat_dist,bp = star
      if 300 <= six <= 1620 and 200 <= siy <= 880:
...
def find_close_stars(star_obj):
   ...
   print("FIND CLOUSE STARS RESULTS:", len(stars), " Close stars found")
   return(stars)
...
def find_best_calibration(cal_fn, orig_cal, json_conf):
   ...
   for row in list_of_cals:
      ...
      #print("AZ/EL:", cp['center_az'], cp['center_el'])
      #print("POS/PX:", cp['position_angle'], cp['pixscale'])
      for star in cp['cat_image_stars']:
