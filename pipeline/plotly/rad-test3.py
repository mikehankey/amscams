...
fig, ax = plt.subplots(subplot_kw={'projection': ccrs.Sinusoidal(central_longitude=0)})
ax.scatter(longitudes_transformed, latitudes, transform=ccrs.PlateCarree(), color='red')

# Explicit list of x coordinates for gridlines
xlocs = list(range(-180, 196, 15))  # Extend the range up to 195

# Draw gridlines using the explicit list
gl = ax.gridlines(crs=ccrs.PlateCarree(), draw_labels=False, linewidth=0.5, color='gray', alpha=0.5, linestyle='--', xlocs=xlocs, ylocs=np.arange(-90, 91, 15))
