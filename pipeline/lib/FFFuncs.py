...
def splice_video(in_file, start, end, outfile=None, que_type="frame"):
   print("Splice:", in_file, start, end, que_type)
   # type = frame or sec 
   ...
   cmd = f"""/usr/bin/ffmpeg -ss {start} -i {in_file} -to {end} -c copy -max_muxing_queue_size 1024 -y {outfile} >/dev/null 2>&1"""
