...
struct  clusterinfo
{
  ...
  long  **block_colsum_even; // Sum of col indices for all clustered exceedances in a block for even rows
                             ...
                             // Work arrays for the Hueckle kernel

  double **hueckel_coef;     // Hueckel coefs blocksize * blocksize ref via hueckel_coef[row,col][basis]
  double  *hueckel_sum;      // Hueckel sum across the ROI per basis of size nbasis
  long     nbasis;           // Number of basis functions in the Hueckle transform kernel
  ...
  double *demsum_downcolumn; // Sum of demean down the rows for each column
...
void  ClusteringInitialization( long maxclusters, long nrows, long ncols, long coldim, long datatype,
                                ...
                                struct clusterinfo *cluster )
{
long    blkrow, hpixel, npixels, krow, kcol, msize;
double  hrow, hcol, deltasq, Q;
    ...
    if( cluster->block_counts_odds == NULL  ||  cluster->block_rowsum_odds == NULL  ||  cluster->block_colsum_odds == NULL  ||
        ...
        printf( " Memory not allocated for block* arrays in ClusteringInitialization \n" );
        Delay_msec(10000);
        exit(1);
    }

    for( blkrow = 0; blkrow < cluster->nblkrows; ++blkrow) {
        cluster->block_counts_odds[blkrow] = (long* )( (cluster->block_counts_odds + cluster->nblkrows) + blkrow * cluster->nblkcols * sizeof(long) / sizeof(long*) );
        cluster->block_rowsum_odds[blkrow] = (long* )( (cluster->block_rowsum_odds + cluster->nblkrows) + blkrow * cluster->nblkcols * sizeof(long) / sizeof(long*) );
        cluster->block_colsum_odds[blkrow] = (long* )( (cluster->block_colsum_odds + cluster->nblkrows) + blkrow * cluster->nblkcols * sizeof(long) / sizeof(long*) );

        cluster->block_counts_even[blkrow] = (long* )( (cluster->block_counts_even + cluster->nblkrows) + blkrow * cluster->nblkcols * sizeof(long) / sizeof(long*) );
        cluster->block_rowsum_even[blkrow] = (long* )( (cluster->block_rowsum_even + cluster->nblkrows) + blkrow * cluster->nblkcols * sizeof(long) / sizeof(long*) );
        cluster->block_colsum_even[blkrow] = (long* )( (cluster->block_colsum_even + cluster->nblkrows) + blkrow * cluster->nblkcols * sizeof(long) / sizeof(long*) );

        if( cluster->block_counts_odds[blkrow] == NULL ||
            cluster->block_rowsum_odds[blkrow] == NULL ||
            cluster->block_colsum_odds[blkrow] == NULL ||
            cluster->block_counts_even[blkrow] == NULL ||
            cluster->block_rowsum_even[blkrow] == NULL ||
            cluster->block_colsum_even[blkrow] == NULL    )  {
            printf( " Memory not allocated for block* rows in ClusteringInitialization \n" );
            Delay_msec(10000);
            exit(1);
        }
    ...
    cluster->demsum_downcolumn = (double*) malloc( ( cluster->ncols ) * sizeof(double) );
    cluster->noisum_downcolumn = (double*) malloc( ( cluster->ncols ) * sizeof(double) );

    if( cluster->sigsum_across_row == NULL  ||  cluster->sigsum_downcolumn == NULL  ||
        cluster->demsum_downcolumn == NULL  ||  cluster->noisum_downcolumn == NULL )  {
    ...
    //--------- Allocate memory for the Hueckel working array (row & col is unwrapped 1st dimension)
    //            This assumes in ClusteringDetector one EITHER does progressive scan OR interleaved
    //            processing but not both.

    if( cluster->interleave == 0 )  msize = blocksize;      //... progressive scan
    else                            msize = blocksize / 2;  //... interleaved


    npixels = msize * msize;
    cluster->nbasis = 7;

    cluster->hueckel_coef = (double**) malloc( ( npixels * sizeof(double*) ) + ( npixels * cluster->nbasis * sizeof(double) ) );
    cluster->hueckel_sum  = (double* ) malloc( cluster->nbasis * sizeof( double ) );

    if( cluster->hueckel_coef == NULL  ||  cluster->hueckel_sum == NULL  )  {
        printf( " Memory not allocated for hueckel in ClusteringInitialization \n" );
        Delay_msec(10000);
        exit(1);
    }


    for( hpixel = 0; hpixel < npixels; ++hpixel) {
         cluster->hueckel_coef[hpixel] = (double* )( (cluster->hueckel_coef + npixels) + hpixel * cluster->nbasis * sizeof(double) / sizeof(double*) );

         if( cluster->hueckel_coef[hpixel] == NULL  )  {
             printf( " Memory not allocated for hueckel_coef rows in ClusteringInitialization \n" );
             Delay_msec(10000);
             exit(1);
         }
    }
    ...
    for( krow=1; krow<=msize; krow++ )  {

         hrow = (double)( 2 * krow - msize - 1 ) / (double)msize;

         for( kcol=1; kcol<=msize; kcol++ )  {

              hcol = (double)( 2 * kcol - msize - 1 ) / (double)msize;

              deltasq = hrow * hrow + hcol * hcol;

              //... within circle of radius blocksize/2

              if( deltasq < 1.0 )  Q = 2.0 * sqrt( ( 1.0 - deltasq ) / ( 3.0 * 3.141592654 ) );
              else                 Q = 0.0;


              //-------- Fill each of the 7 basis vectors for this pixel position

              cluster->hueckel_coef[hpixel][0] = Q * ( 5.0 * deltasq - 2.0 );
              cluster->hueckel_coef[hpixel][1] = Q * hcol * 3.0;
              cluster->hueckel_coef[hpixel][2] = Q * hrow * 3.0;
              cluster->hueckel_coef[hpixel][3] = Q * ( hcol * hcol - hrow * hrow ) * 3.0 * sqrt(2.0);
              cluster->hueckel_coef[hpixel][4] = Q * hcol * hrow * 6.0 * sqrt(2.0);
              cluster->hueckel_coef[hpixel][5] = Q * ( 2.0 * deltasq - 1.0 ) * hcol * 3.0 * sqrt(5.0);
              cluster->hueckel_coef[hpixel][6] = Q * ( 2.0 * deltasq - 1.0 ) * hrow * 3.0 * sqrt(5.0);
...
unsigned char  *uc_image = NULL, *uc_tmean = NULL, *uc_sigma = NULL;  //--- pointers for any of the data types allowed
unsigned short *us_image = NULL, *us_tmean = NULL, *us_sigma = NULL;
short          *ss_image = NULL, *ss_tmean = NULL, *ss_sigma = NULL;
long           *sl_image = NULL, *sl_tmean = NULL, *sl_sigma = NULL;
float          *fl_image = NULL, *fl_tmean = NULL, *fl_sigma = NULL;
double         *db_image = NULL, *db_tmean = NULL, *db_sigma = NULL;
...
long    startrow, startcol, row, col, rcindex, msize, hpixel, kbasis;
...
double  Hueckel_angle, e2, e3, e4, e5, g1, g2, ccx, ccy;
double  pixval, xmean, demean, signal, noise, stddev, variance;
double  SsqN, SsqNdB, p, entropy;

unsigned char  *uc_image = NULL, *uc_tmean = NULL, *uc_sigma = NULL;
unsigned short *us_image = NULL, *us_tmean = NULL, *us_sigma = NULL;
short          *ss_image = NULL, *ss_tmean = NULL, *ss_sigma = NULL;
long           *sl_image = NULL, *sl_tmean = NULL, *sl_sigma = NULL;
float          *fl_image = NULL, *fl_tmean = NULL, *fl_sigma = NULL;
         ...
         if( krowmode == 2  &&  cluster->interleave == 0 )  continue;  // skip odd & even row detection if progressive scan
              ...
              for( jpeak=0; jpeak<npeaks; jpeak++ )  {
                   ...
                   for( col=startcol; col<startcol+msize; col++ )  {
                        ...
                        cluster->demsum_downcolumn[col] = 0.0;
                   ...
                   //            and imaginary sums, zero the Hueckel sums.
                   ...
                   for( kbasis=0; kbasis<cluster->nbasis; kbasis++ )  cluster->hueckel_sum[kbasis] = 0.0;
                   ...
                   for( row=startrow; row<startrow+msize; row++ )  {
                        ...
                        for( col=startcol; col<startcol+msize; col++ )  {
                             ...
                             noise    = 1.0 / variance;
                             demean   = fmax( 0.0, pixval - xmean );
                             signal   = demean * noise;

                             cluster->sigsum_downcolumn[col] += signal;
                             cluster->demsum_downcolumn[col] += demean;
                             ...
                             //........ Apply the Hueckle kernel to the adjusted signal (grey level)

                             for( kbasis=0; kbasis<cluster->nbasis; kbasis++ )  {

                                  cluster->hueckel_sum[kbasis] += signal * cluster->hueckel_coef[hpixel][kbasis];
                   ...
                   for( col=startcol; col<startcol+msize; col++ )  {
                        ...
                        demean += cluster->demsum_downcolumn[col];
                   ...
                   //-------- Compute the Hueckle line orientation

                   e2 = sqrt(2.0/3.0) * cluster->hueckel_sum[3];
                   e3 = sqrt(2.0/3.0) * cluster->hueckel_sum[4];
                   e4 = sqrt(8.0/9.0) * cluster->hueckel_sum[0] * cluster->hueckel_sum[3]
                              + 0.5 * ( cluster->hueckel_sum[1] * cluster->hueckel_sum[1]
                                      - cluster->hueckel_sum[2] * cluster->hueckel_sum[2]
                                      + cluster->hueckel_sum[5] * cluster->hueckel_sum[5]
                                      - cluster->hueckel_sum[6] * cluster->hueckel_sum[6] );
                   e5 = sqrt(8.0/9.0) * cluster->hueckel_sum[0] * cluster->hueckel_sum[4]
                                      + cluster->hueckel_sum[1] * cluster->hueckel_sum[2]
                                      + cluster->hueckel_sum[5] * cluster->hueckel_sum[6];

                   g1 = signum( e2*e4 + e3*e5 ) * sqrt( e2*e2 + e3*e3 );
                   ccx = g1 * e2 + e4;
                   ccy = g1 * e3 + e5;

                   g2 = sqrt( ccx * ccx + ccy * ccy );
                   ccx = ccx / g2;
                   ccy = ccy / g2;

                   if( ccx + 1.0 > 0.5e-10 )  Hueckel_angle = -180.0 * atan2( ccy, ccx + 1.0 ) / 3.141592654;
                   else                       Hueckel_angle = -180.0 * atan2( signum(ccy), ccx + 1.0 ) / 3.141592654;
                   ...
                   if( SsqNdB >= cluster->SsqNdB_threshold )  {
                       ...
                       switch ( krowmode )
                       {
                            case 1:  //... Interleaved (odd rows only)
                                ...
                                cluster->measurements_odds[kpeak].sumdemean     = demean;
                            ...
                            case 2:  //... Interleaved (even rows only)
                                ...
                                cluster->measurements_even[kpeak].sumdemean     = demean;
                            ...
                            default: //... Progressive scan (all rows)
                                ...
                                cluster->measurements_full[kpeak].sumdemean     = demean;
...
void  ClusteringFreeMemory( struct clusterinfo *cluster )
{
    ...
    free( cluster->demsum_downcolumn );
