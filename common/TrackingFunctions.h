...
//  2018-04-10  Gural       Merged a CLOSED track that was a duplicate of a FIRM track
//  2018-11-26  Gural       Reconciled Tracking codes with EMCCD and CAMS
//
//#######################################################################################
...
struct   trackerinfo
{
	...
	double    multi_integ;         // Multi-frame integrated demeaned signal
	double    multi_entropy;       // Multi-frame average entropy
	double    multi_PCD_angle;     // Multi-frame average PCD angle in degrees
	double    multi_Hueckel_angle; // Multi-frame average Hueckel angle in degrees
...
struct  trackerinfo  *trackers;
	...
	for( ktrack=0; ktrack<maxtracks; ktrack++ )  {
	     ...
	     if( trak->detectflags_history == NULL )  {
	         printf("ERROR allocating memory on track %li for detectflags_history in TrackingInitialization\n", ktrack );
			 Delay_msec(5000);
		     exit(1);
	     }
	     ...
	     if( trak->detected == NULL )  {
	         printf("ERROR allocating memory on track %li for attributeinfo detected in TrackingInitialization\n", ktrack );
			 Delay_msec(5000);
		     exit(1);
	     }
...
//###################################################################################
// The TrackingResetAllTracks function is called if there is a large gap in time
// and the trackers are wholly out of date. All tracks set to NOTRACK and history
// cleaned up.
//
// INPUTS:
//    *trackers      Pointer to maxtracks vector of trackerinfo structures
...
void  TrackingResetAllTracks(struct trackerinfo *trackers)
{
	long    ktrack;
	struct  trackerinfo  *trak;
	...
	for (ktrack = 0; ktrack<trackers->maxtracks; ktrack++) {
...
//###################################################################################
// The TrackingCloseTracks function is called to change the status of all FIRM and 
// TROUBLED tracks to a CLOSED status so calls to TrackingSaver will extract
// and report on all non-spinup tracks if the tracking process is ending.
// 
//
// INPUTS:
//    *trackers      Pointer to maxtracks vector of trackerinfo structures
...
void  TrackingCloseTracks(struct trackerinfo *trackers)
{
	long    ktrack;
	struct  trackerinfo  *trak;
	...
	for (ktrack = 0; ktrack<trackers->maxtracks; ktrack++) {
		...
		if (trak->status >= FIRM)  trak->status = CLOSED;
...
//###################################################################################
// The TrackingGetNextAvailable function is called whenever a new track needs to
// be pulled from the available tracker pool. If no free tracks are available, then
// the returned trackerinfo pointer is NULL. Other wise the pointer is to the 
// specific track from the vector of trackers
//
// INPUTS:
//    *trackers      Pointer to maxtracks vector of trackerinfo structures
//
// OUTPUT:
//    *trak          Pointer to an trackerinfo structure with status = NOTRACK
//
//  DEPENDENCIES:
//     None
//
...
struct trackerinfo*  TrackingGetNextAvailable( struct trackerinfo *trackers )
{
long                ktrack;
struct trackerinfo *trak;
	...
	for( ktrack=0; ktrack<trackers[0].maxtracks; ktrack++ )  {
		...
		if( trak->status == NOTRACK )  return( trak );

	}

	printf(" WARNING: Run out of available trackers in TrackingFunctions.h \n");	
...
int k;
    ...
    trak2->multi_integ         = trak1->multi_integ;
    trak2->multi_entropy       = trak1->multi_entropy;
    trak2->multi_PCD_angle     = trak1->multi_PCD_angle;
    trak2->multi_Hueckel_angle = trak1->multi_Hueckel_angle;
	...
	for( k=0; k<trak1->numhist; k++ )  trak2->detectflags_history[k] = trak1->detectflags_history[k];


    for( k=0; k<trak1->nummeas; k++ )  {
		...
		trak2->detected[k].sumdemean     = trak1->detected[k].sumdemean;
...
//###################################################################################
// The TrackingMerge function concatenates the early portion of trak1 to trak2 and
// places the output in trakmerge. Trakmerge has the latest status and attributes
// of trak2. Obtain the index "kmeas1_common", where the two tracks first have common 
// measurements, by first calling TrackingCommonMeasStartIndex.
//===================================================================================

void   TrackingMerge( int                   kmeas1_common, 
                      struct  trackerinfo  *trak1, 
					  struct  trackerinfo  *trak2, 
					  struct  trackerinfo  *trakmerge )
{
int  k, kmeas, kmeas1_start;


    //------ Do not overrun maxhistory buffer dimension for detectflags

    if( trak2->numhist + kmeas1_common <= trak2->maxhistory )  
		kmeas1_start = 0;
	else
		kmeas1_start = kmeas1_common - ( trak2->maxhistory - trak2->numhist );


	//------ copy scalers from trak2 to trakmerge

    trakmerge->maxtracks       = trak2->maxtracks;
    trakmerge->maxhistory      = trak2->maxhistory;
    trakmerge->status          = trak2->status;
    trakmerge->numhist         = trak2->numhist + kmeas1_common - kmeas1_start ;
    trakmerge->nummeas         = trak2->nummeas + kmeas1_common - kmeas1_start;
    trakmerge->mfirm           = trak2->mfirm;
    trakmerge->nfirm           = trak2->nfirm;
    trakmerge->mdrop           = trak2->mdrop;
    trakmerge->ndrop           = trak2->ndrop;
    trakmerge->ndown           = trak2->ndown;
    trakmerge->alpha           = trak2->alpha;
    trakmerge->beta            = trak2->beta;
    trakmerge->timeupdate      = trak2->timeupdate;
    trakmerge->timemeas        = trak2->timemeas;
    trakmerge->rowmeas         = trak2->rowmeas;
    trakmerge->rowfilt         = trak2->rowfilt;
    trakmerge->rowrate         = trak2->rowrate;
    trakmerge->rowpred         = trak2->rowpred;
    trakmerge->colmeas         = trak2->colmeas;
    trakmerge->colfilt         = trak2->colfilt;
    trakmerge->colrate         = trak2->colrate;
    trakmerge->colpred         = trak2->colpred;

    trakmerge->multi_SNRdB         = trak2->multi_SNRdB;
    trakmerge->multi_BNRdB         = trak2->multi_BNRdB;
    trakmerge->multi_integ         = trak2->multi_integ;
    trakmerge->multi_entropy       = trak2->multi_entropy;
    trakmerge->multi_PCD_angle     = trak2->multi_PCD_angle;
    trakmerge->multi_Hueckel_angle = trak2->multi_Hueckel_angle;

    trakmerge->multi_rowstart  = trak2->multi_rowstart;
    trakmerge->multi_colstart  = trak2->multi_colstart;
    trakmerge->multi_rowspeed  = trak2->multi_rowspeed;
    trakmerge->multi_colspeed  = trak2->multi_colspeed;
    trakmerge->multi_linearity = trak2->multi_linearity;
    trakmerge->multi_modelerr  = trak2->multi_modelerr;


	//------ First copy the early part of the trak1 vectors to trakmerge.
	//          Assume detectflags are all detects for the trak1 copy.

	k = 0;

	for( kmeas=kmeas1_start; kmeas<kmeas1_common; kmeas++ )  {
		
		trakmerge->detectflags_history[k]    = WAS_DETECTED;

		trakmerge->detected[k].time          = trak1->detected[kmeas].time;
		trakmerge->detected[k].rowcentroid   = trak1->detected[kmeas].rowcentroid;
		trakmerge->detected[k].colcentroid   = trak1->detected[kmeas].colcentroid;
		trakmerge->detected[k].nexceed       = trak1->detected[kmeas].nexceed;
		trakmerge->detected[k].nsaturated    = trak1->detected[kmeas].nsaturated;
		trakmerge->detected[k].npixels       = trak1->detected[kmeas].npixels;
		trakmerge->detected[k].sumdemean     = trak1->detected[kmeas].sumdemean;
		trakmerge->detected[k].sumsignal     = trak1->detected[kmeas].sumsignal;
		trakmerge->detected[k].sumnoise      = trak1->detected[kmeas].sumnoise;
		trakmerge->detected[k].SsqNdB        = trak1->detected[kmeas].SsqNdB;
		trakmerge->detected[k].entropy       = trak1->detected[kmeas].entropy;
		trakmerge->detected[k].PCD_angle     = trak1->detected[kmeas].PCD_angle;
		trakmerge->detected[k].PCD_width     = trak1->detected[kmeas].PCD_width;
		trakmerge->detected[k].Hueckel_angle = trak1->detected[kmeas].Hueckel_angle;
	...
	//------ Next copy the full detectflags_history of trak2 to trakmerge

	for( kmeas=0; kmeas<trak2->numhist; kmeas++ )  {
		
		trakmerge->detectflags_history[k+kmeas] = trak2->detectflags_history[kmeas];

	}


	//------ Finally copy all of trak2 vectors to trakmerge

	for( kmeas=0; kmeas<trak2->nummeas; kmeas++ )  {
		
		trakmerge->detected[k].time          = trak2->detected[kmeas].time;
		trakmerge->detected[k].rowcentroid   = trak2->detected[kmeas].rowcentroid;
		trakmerge->detected[k].colcentroid   = trak2->detected[kmeas].colcentroid;
		trakmerge->detected[k].nexceed       = trak2->detected[kmeas].nexceed;
		trakmerge->detected[k].nsaturated    = trak2->detected[kmeas].nsaturated;
		trakmerge->detected[k].npixels       = trak2->detected[kmeas].npixels;
		trakmerge->detected[k].sumdemean     = trak2->detected[kmeas].sumdemean;
		trakmerge->detected[k].sumsignal     = trak2->detected[kmeas].sumsignal;
		trakmerge->detected[k].sumnoise      = trak2->detected[kmeas].sumnoise;
		trakmerge->detected[k].SsqNdB        = trak2->detected[kmeas].SsqNdB;
		trakmerge->detected[k].entropy       = trak2->detected[kmeas].entropy;
		trakmerge->detected[k].PCD_angle     = trak2->detected[kmeas].PCD_angle;
		trakmerge->detected[k].PCD_width     = trak2->detected[kmeas].PCD_width;
		trakmerge->detected[k].Hueckel_angle = trak2->detected[kmeas].Hueckel_angle;
	...
	//------ Test we have the right number of measurements

	if( trakmerge->nummeas != k )  {
	    printf("ERROR mismatch in number of measurements in TrackingMerge\n");
		Delay_msec(5000);
		exit(1);
	}
...
//###################################################################################
// The TrackingCommonMeasStartIndex function searches for common measurements
// between trak1 to trak2 and returns the index of the first measurement in trak1
// that is in common with trak2. The number of measurements in common must meet the
// minimum requirement set by the calling function. Returns 9999 when no measurements
// are in common.
//===================================================================================

long   TrackingCommonMeasStartIndex( int                   min_common_count,
                                    struct  trackerinfo  *trak1, 
					                struct  trackerinfo  *trak2  )
{
long  kmeas1, kmeas2, kmeas_common, common_count;


	//====== Check every measurement in trak1 to trak2 to identify common measurements

    kmeas_common = 9999;  //... Assume nothing in common

    common_count = 0;

	for( kmeas1=0; kmeas1<trak1->nummeas; kmeas1++ )  {

		for( kmeas2=0; kmeas2<trak2->nummeas; kmeas2++ )  {

			if( ( trak1->detected[kmeas1].time        == trak2->detected[kmeas2].time        )  &&
                ( trak1->detected[kmeas1].rowcentroid == trak2->detected[kmeas2].rowcentroid )  && 
                ( trak1->detected[kmeas1].colcentroid == trak2->detected[kmeas2].colcentroid )     )  {

					//------ For the first measurement in common - save the start index from trak1

					if( common_count == 0 )  kmeas_common = kmeas1;
	...
	//====== Check if number of measurements in common meets the minimum count

	if( common_count < min_common_count )  kmeas_common = 9999;  //... Set to nothing in common
...
void    TrackingAllMeasurements( long   nmeas,
	                             ...
	                             struct trackerinfo  *trackers )
{
long                 kmeas, kmeas_common, ktrack, jtrack, maxtracks, num_assoc;
unsigned char       *measurement_assoc;
double              *rowmeas, *colmeas;
struct trackerinfo  *trak;
struct trackerinfo  *traktest;
struct trackerinfo  *trakmerge;
    ...
    for( kmeas=0; kmeas<nmeas; kmeas++ )  {
		 ...
		 //            and initiate a new track with this unassociated measurement
		 //            and flag the history as a detection.

		 trak = TrackingGetNextAvailable( trackers );

		 if( trak != NULL )  TrackingUpdate( WAS_DETECTED, &measurements[kmeas], time_of_measurements, trak );
	...
	//======== Check CLOSED tracks for possible merge with an active FIRM, TROUBLED, or CLOSED track

	if( TrackingGetNextAvailable( trackers ) != NULL )  { 
	    ...
	    for( ktrack=0; ktrack<maxtracks; ktrack++ )  {
		     ...
		     if( trak->status == CLOSED )  {
				 ...
				 //------ Locate a FIRM, TROUBLED, or CLOSED track

	             for( jtrack=0; jtrack<maxtracks; jtrack++ )  {

					 if( jtrack == ktrack )  continue;  //... skip if same track index

	                 traktest = &trackers[jtrack];

		             if( traktest->status >= FIRM )  {

						 //------ Get the measurement point index in the CLOSED "trak" that
						 //          is in common between track measurements. Must be at
						 //          least 2 measurments in common, else no merge.

						 kmeas_common = TrackingCommonMeasStartIndex( 2, trak, traktest );


						 if( kmeas_common == 9999 )  continue;  //... Not in common, goto next FIRM


						 if( kmeas_common <= 0 )  {  //... Remove trak, it is within traktest 
						 ...
						 if( kmeas_common >  0 )  {  //... Generate a new combined track
							 
							 trakmerge = TrackingGetNextAvailable( trackers );
							 
							 TrackingMerge( kmeas_common, trak, traktest, trakmerge );

							 //...... Tracks released back to trackers pool and reset

	                         trak->status = NOTRACK;

			                 TrackingSetup( trak );

	                         traktest->status = NOTRACK;

			                 TrackingSetup( traktest );

							 break;

						 }

					 } //... end of IF track is at least firm FIRM

				 } //... end of loop testing all tracks for at least FIRM status


				 if( trak->status == NOTRACK )  break;  // Track was removed, goto next CLOSED


			 } //... end of IF track status is CLOSED

		} //... end of loop testing all tracks for CLOSED status

	} //... end of test for available unused tracks  
...
//###################################################################################
//  The TrackingMultiFrameAverages function averages several attributes across
//  a series of "n" measurements spanning multiple frames. 
//
// INPUTS:
//     track        Pointer to the trackinfo structure containing:
//       .nummeas                      Number of measurements in this track
...
// OUTPUT:
//     track        Pointer to the trackinfo structure containing:
//       .multi_SNRdB             Multi-frame SNR in dB 10*log10(signal^2/noise)
//       .multi_BNRdB             Multi-frame BNR in dB 10*log10(exceedances/total_pixels)
//       .multi_integ             Multi-frame integrated demeaned signal
//       .multi_PCD_angle         Multi-frame average PCD angle in degrees
//       .multi_Hueckel_angle     Multi-frame average Hueckel angle in degrees
//       .multi_entropy           Multi-frame average entropy
//
//===================================================================================

void     TrackingMultiFrameAverages(struct  trackerinfo  *trak)
{
	long      knumer, kdenom, kmeas;
	double    signal, demean, noise, SNR;
	double    xpsum, ypsum, xhsum, yhsum, esum;
	...
	for (kmeas = 0; kmeas<trak->nummeas; kmeas++) {
		...
		demean += trak->detected[kmeas].sumdemean;
		noise += trak->detected[kmeas].sumnoise;

		xpsum += cos(trak->detected[kmeas].PCD_angle / 57.29577951);
		ypsum += sin(trak->detected[kmeas].PCD_angle / 57.29577951);

		xhsum += cos(trak->detected[kmeas].Hueckel_angle / 57.29577951);
		yhsum += sin(trak->detected[kmeas].Hueckel_angle / 57.29577951);
	...
	trak->multi_integ = demean;


	trak->multi_PCD_angle = 57.29577951 * atan2(ypsum, xpsum);  //...deg

	trak->multi_Hueckel_angle = 57.29577951 * atan2(yhsum, xhsum);  //...deg
...
struct trackerinfo  *trak;
	...
	for( ktrack=0; ktrack<maxtracks; ktrack++ )  {
		 ...
		 if( trak->status == CLOSED )  {
			 ...
			 //...... Compute multi-frame parameters

			 TrackingMultiFrameAverages(trak);
...
//###################################################################################
// The TrackingScale function is called to scale track products by a decimation
