...
import sys
from caliblib import load_json_file, save_json_file, distort_xy, distort_xy_new
from detectlib import calc_dist , cfe
...
def get_fov_stars(params, mapped_stars_file, dimension, other_poly, info_only = 0):
   ...
   if info_only == 1:
      return(matched_stars, avg_lines, cost, possible_stars)
   else:
      if len(lines) > 0:
         return(cost)
      else:
...
#x_poly = np.array(( 1.13290652e-07,  2.19009809e-04,  3.96925932e-07,  1.01200200e-06, -1.42637592e-06,  8.02963978e-06,  2.03924532e-09,  6.33964060e-08, -3.36684502e-08, -1.89335760e-08,  2.28019419e-05,  3.47063883e-08), dtype=np.float64)
#x_poly = np.zeros(shape=(12,), dtype=np.float64)
#y_poly = np.zeros(shape=(12,), dtype=np.float64)
